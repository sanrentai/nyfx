...
public class ChangePasswordController extends BusinessBaseController {
    ...
    public String onSaveModify() {
        ...
        TUser tUser = this.accountManager.getTUser();

        ///更新密码
        ExecuteResult result = this.bizLogic.saveChange(tUser, SecurityUtils.MD5(this.vm.getPassword()));

        this.addMessage(result.createMessage());

//        if (result.isSuccess()) {
//            tUser.setIsFirstTimeLogin(false);
