...
public class LoginController extends BusinessBaseController {
    ...
    @PostConstruct
    public void init() {
        tUserList=bizLogic.getTUserList();
    }
    ...
    public String login() {
        ...
        try {
            LOG.info(loginId);
            LOG.info(password);
            LOG.info(SecurityUtils.MD5(this.password).toUpperCase());
            /// ログイン情報を検索する。
            TUser tUser = bizLogic.searchTUser( this.loginId, SecurityUtils.MD5(this.password).toUpperCase());

            ///Login成功 or 失败
            this.accountManager.setLoginIsSuccess(tUser != null);

            ///Login者
            this.accountManager.setTUser(tUser);
        ...
        /// 是否为冻结的用户
//        if (this.accountManager.getTUser().isIsLockUser()) {
